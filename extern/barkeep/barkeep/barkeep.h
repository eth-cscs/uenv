...
class AsyncDisplayer {
  ...
  public:
    ...
    bool running() const {
        return displayer_thread_ != nullptr;
    }
...
class BaseDisplay {
  ...
  public:
    ...
    std::ostream& out() const {
        return displayer_->out();
    }

    /// Return true if the display is running.
    bool running() const {
        return displayer_->running();
    }
...
template <typename Progress> class Speedometer {
  private:
  ...
  public:
    double speed() {
        ...
        ValueType progress_copy =
            *progress_; // to avoid progress_ changing below
...
class CounterDisplay : public BaseDisplay {
  ...
  protected:
    ...
    long render_(bool /*redraw*/ = false,
        ...
        if (not format_.empty()) {
            ...
            if (speedom_) {
                fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                           ...
                           "green"_a = green, "yellow"_a = yellow,
                           "blue"_a = blue, "magenta"_a = magenta,
                           "cyan"_a = cyan, "reset"_a = reset);
            } else {
                fmt::print(out(), fmt::runtime(format_), "value"_a = progress,
                           "red"_a = red, "green"_a = green,
                           "yellow"_a = yellow, "blue"_a = blue,
                           "magenta"_a = magenta, "cyan"_a = cyan,
                           "reset"_a = reset);
            }
  ...
  public:
    ...
    CounterDisplay(Progress* progress, const CounterConfig& cfg = {})
        : BaseDisplay(cfg.out, as_duration(cfg.interval), cfg.message,
          ...
          progress_(progress), speed_unit_(cfg.speed_unit) {
        if (cfg.speed) {
            speedom_ =
...
template <typename Progress> class ProgressBarDisplay : public BaseDisplay {
  ...
  public:
    ...
    ProgressBarDisplay(Progress* progress,
        ...
        : BaseDisplay(cfg.out, as_duration(cfg.interval), cfg.message,
                      cfg.format.empty() ? "" : cfg.format + " ", cfg.no_tty),
          progress_(progress), speed_unit_(cfg.speed_unit), total_(cfg.total) {
  ...
  public:
    ...
    class Iterator {
      ...
      public:
        Iterator(typename Container::iterator it,
                 typename Container::iterator end, ProgressType& idx,
    ...
    IterableBar(Container& container,
        ...
        : container_(container), idx_(std::make_shared<ProgressType>(0)),
          bar_(std::make_shared<Bar>(&*idx_,
                                     ProgressBarConfig<ValueType>{
                                         cfg.out, container.size(), cfg.format,
