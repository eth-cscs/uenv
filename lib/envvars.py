...
def read_dictionary(d: dict, env: Optional[EnvVarSet] = None) -> EnvVarSet:
    if env is None:
        env = EnvVarSet()

    if "scalar" in d:
        for name, value in d["scalar"].items():
            env.set_scalar(name, value)

    if "list" in d:
        for name, updates in d["list"].items():
            for u in updates:
                if u["op"] == "set":
                    env.set_list(name, u["value"], EnvVarOp.SET)
                elif u["op"] == "prepend":
                    env.set_list(name, u["value"], EnvVarOp.PREPEND)
                elif u["op"] == "append":
                    env.set_list(name, u["value"], EnvVarOp.APPEND)
                # just ignore updtes with invalid "op"
