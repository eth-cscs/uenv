...
// Download the squashfs layer of `manifest` into <store>/store.squashfs and
// self-verify: the file is re-digested and checked against the layer digest, so
// a truncated or corrupt download is caught locally. `manifest` is the image
// manifest already fetched via client::get_manifest. An optional abort
// predicate, polled during the download, cancels it (for Ctrl-C handling).
...
pull_meta(client& c, const std::string& manifest_digest,
