...
int main(int argc, char** argv, char** envp) {
    ...
    // occurrence (repeat the flag for more than one).
    cli.add_option(
           "--tmpfs", tmpfs_arg,
           "a comma separated list of tmpfs mounts, <mount>[:<size>] (size "
           "in bytes); repeat the flag for more than one")
        ->allow_extra_args(false);
    cli.add_option("--bind-mount", bind_mounts_arg,
                   "a comma separated list of bind mounts, <src>:<dst>; "
